...
export function StoreWorkspace() {
  ...
  if (!store) return <div className="store-empty"><span className="state-icon"><Store/></span><h1>You have not built a store yet</h1><p>Shortlist products in Discover or let Launchly assemble a balanced demo portfolio.</p><div><Button variant="coral" onClick={() => router.push("/discover")}><PackagePlus/>Browse trending</Button><Button onClick={() => createStore(trendingProducts.slice(0, 4).map((item) => ({ id: item.id, key: item.key, category: item.category, name: item.name, description: item.description, price: item.price, successScore: item.successScore, monthlyProfit: item.monthlyProfit, startupCost: item.startupCost, image: item.image, trend: item.trend, sourceType: "simulation" })))}><Bot/>Build one for me</Button></div><Badge tone="mint">Private Supabase portfolio · simulated product data</Badge></div>;
  ...
  return <>
    <div className="store-hero"><Badge tone="neutral">Private portfolio · Supabase</Badge><h1>{store.brand}</h1><p>{store.description}</p><small>Combined scenario profit</small><strong>{money(monthly)} <span>/month</span></strong></div>
