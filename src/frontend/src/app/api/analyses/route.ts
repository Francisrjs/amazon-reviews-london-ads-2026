...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({
        error: response.status === 422 ? "The model rejected these product details." : body?.detail ?? body?.error ?? "The backend is unavailable.",
      }, { status: response.status });
    }
    const model = fastApiAnalysisSchema.safeParse(body);
    if (!model.success) return NextResponse.json({ error: "The backend returned an unexpected response." }, { status: 502 });
    return NextResponse.json(model.data);
  } catch (error) {
    const timeout = error instanceof Error && error.name === "TimeoutError";
    return NextResponse.json({ error: timeout ? "The model took too long to respond." : "The backend is unavailable." }, { status: 503 });
  }
}
